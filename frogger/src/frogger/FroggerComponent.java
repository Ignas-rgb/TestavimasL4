...
public class FroggerComponent extends JComponent implements KeyListener, Runnable {
    ...
    public int askInputError = 0;
    public int askInputSuccess = 0;
    public int endGameAfterVicotory = 0;
    ...
    public void update() throws InterruptedException {
        ...
        if (engine.getState() == FroggerState.WON) {
            ...
            if (level >= levels.size()) {
            	if(askInputSuccess == 1) {
            		askForSuccessInput();
            	}
            	endGameAfterVicotory = 1;
            	if(endGameAfterVicotory == 1) {
            		System.exit(0);
            	}
            }
            engine = new FroggerLevelEngine(levels.get(level));
